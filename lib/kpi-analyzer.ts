...
function generateRecommendations(kpiGroups: Record<string, KpiDefinition[]>, conflicts: any[]) {
  // Create a map of conflicted metric names for quick lookup
  const conflictedMetrics = new Set(conflicts.map((conflict) => conflict.metricName.toLowerCase().trim()))

  return Object.entries(kpiGroups)
    .filter(([normalizedName, kpis]) => {
      // Include metrics that have conflicts or are used by multiple teams
      return conflictedMetrics.has(normalizedName) || kpis.length > 1
    })
    .map(([kpiName, kpis]) => {
...
function generateEquivalentTerms(kpiName: string) {
  ...
  const equivalentTermsMap: Record<string, string[]> = {
    ...
    "Engagement Rate": ["Engagement", "Interaction Rate"],
    "Active Users": ["DAU", "MAU", "WAU"],
    "Retention Rate": ["User Retention", "Customer Retention"],
    Revenue: ["Sales", "Income"],
    Leads: ["Prospects", "Opportunities"],
  }

  // Find a case-insensitive match
  for (const [key, values] of Object.entries(equivalentTermsMap)) {
